...
import {
  getMessagesByChatFirebase,
  saveMessageFirebase,
} from "../../../services/firebase/controller/message-firebase";
import {
  deleteChatFirebase,
  saveChatFirebase,
  updateChatFirebase,
} from "../../../services/firebase/controller/chat-firebase";
...
export const Chat = () => {
  ...
  const [chats, setChats] = useState({ size: 0, empty: true, docs: [] });
  const [messages, setMessages] = useState({ size: 0, empty: true, docs: [] });
  const [users, setUsers] = useState({ size: 0, empty: true, docs: [] });
  ...
  useEffect(() => {
    ...
    // if (isUserSignedIn()) { // Todo
    //   getChatsByMember(userAccount.uid)
  ...
  useEffect(() => {
    if (selectedChat) {
      if (isUserSignedIn()) {
        getMessagesByChatFirebase(selectedChat.id)
  ...
  const createChatHandle = async (chat, chatDp = undefined) => {
    try {
      if (isUserSignedIn()) {
        const chatCreated = await saveChatFirebase(chat);
        if (chatCreated) {
          setChats(...chats, chatCreated);
          if (chatDp) {
  ...
  const createMessageHandle = async (message) => {
    try {
      if (isUserSignedIn()) {
        const messageCreated = await saveMessageFirebase(message);
        if (messageCreated) {
  ...
  const updateChatHandle = async (chatUpdate) => {
    try {
      if (isUserSignedIn()) {
        if (selectedChat) {
          const chatUpdated = await updateChatFirebase(
            selectedChat.id,
            chatUpdate
          );
          if (chatUpdated) {
            setChats(...chats, chatUpdated);
  ...
  const handleChatDelete = async () => {
    try {
      if (isUserSignedIn()) {
        await deleteChatFirebase(selectedChat.id)
          .then((chatDeleted) => {
            if (chatDeleted) {
              setChats(chats - chatDeleted);
